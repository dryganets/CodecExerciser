...
class SpeedControlCallback : MoviePlayer.FrameCallback {
    ...
    override fun preRender(presentationTimeUsec: Long) { // For the first frame, we grab the presentation time from the video
        ...
        if (mPrevMonoUsec == 0L) { // Latch current values, then return immediately.
            ...
            frameDelta =
                if (mFixedFrameDurationUsec != 0L) { // Caller requested a fixed frame rate.  Ignore PTS.
            ...
            if (frameDelta < 0) {
                ...
                frameDelta = 0
            } else if (frameDelta == 0L) { // This suggests a possible bug in movie generation.
                Log.i(
                    TAG,
